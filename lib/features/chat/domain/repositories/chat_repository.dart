...
class ChatRepository implements ChatRepositoryInterface {
  ...
  Future<ConversationsModel?> getConversationList(int offset) async {
    ...
    Response response = await apiClient.getData(
        '${AppConstants.getConversationListUri}?token=${_getUserToken()}&offset=$offset&limit=10');
  ...
  Future<MessageModel?> getMessages(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.getMessageListUri}?${conversationID != null ? 'conversation_id' : userType == AppConstants.user ? 'user_id' : 'vendor_id'}=${conversationID ?? userId}&token=${_getUserToken()}&offset=$offset&limit=10');
