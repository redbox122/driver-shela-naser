...
class ChatService implements ChatServiceInterface {
  ...
  @override
  Future<MessageModel?> sendMessage(String message, List<MultipartBody> file,
      int? conversationId, int? userId, String userType) async {
    return await chatRepositoryInterface.sendMessage(
        message, file, conversationId, userId, userType);
  }

  @override
  Future<MessageModel?> processGetMessage(int offset,
      NotificationBodyModel notificationBody, int? conversationID) async {
    MessageModel? messageModel;
    if (notificationBody.customerId != null ||
        notificationBody.type == AppConstants.user) {
      messageModel = await getMessages(offset, notificationBody.customerId,
          AppConstants.user, conversationID);
    } else if (notificationBody.vendorId != null ||
        notificationBody.type == AppConstants.vendor) {
      messageModel = await getMessages(offset, notificationBody.vendorId,
          AppConstants.vendor, conversationID);
    }
    return messageModel;
  }
  ...
  @override
  Future<MessageModel?> processSendMessage(
    ...
    if (notificationBody != null &&
        (notificationBody.customerId != null ||
            notificationBody.type == AppConstants.user)) {
      messageModel = await sendMessage(message, chatImage, conversationId,
          notificationBody.customerId, AppConstants.customer);
    } else if (notificationBody != null &&
        (notificationBody.vendorId != null ||
            notificationBody.type == AppConstants.vendor)) {
      messageModel = await sendMessage(message, chatImage, conversationId,
          notificationBody.vendorId, AppConstants.vendor);
    }
    return messageModel;
  }
}
