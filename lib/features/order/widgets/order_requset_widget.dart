...
import '../../../common/widgets/add_request_price.dart';
...
class OrderRequestWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => OrderDetailsScreen(orderId: orderModel.id, isRunningOrder: true, orderIndex: index))),
...
if(orderModel.module_id!=8)
                ...
                Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text(
                     orderModel.module_id==8? "serviceRequest".tr:'deliver_to'.tr, maxLines: 1, overflow: TextOverflow.ellipsis,
                      style:  robotoMedium.copyWith(fontSize: Dimensions.fontSizeSmall),
                    ),
                    const SizedBox(height: Dimensions.paddingSizeExtraSmall),
                ...
                Expanded(
                  child: Row(children: [
                    ...
                    Expanded(child: CustomButtonWidget(
                      ...
                      buttonText:orderModel.module_id==8? "addRequest".tr:'accept'.tr,
                      fontSize: Dimensions.fontSizeDefault,
                      onPressed: () {
                        orderModel.module_id==8?Get.dialog(AddRequestPrice(
                          icon: Images.warning, title: 'are_you_sure_to_accept'.tr,
                          description:"you_want_to_accept_this_service".tr,
                          onYesPressed: () {
                            print(orderModel.groupOrder);
                            orderController.addRequest(orderModel.id).then((isSuccess) {
                              if(isSuccess) {
                                Get.find<OrderController>().getLatestOrders();

                                Navigator.pop(context);
                                Get.showSnackbar(const GetSnackBar(title: "تم تقديم عرضك بنجاح",message: "تم تقديم عرضك بنجاح",duration: Duration(milliseconds: 600),));
                              }else {
                                Get.find<OrderController>().getLatestOrders();
                              }
                            });
                          },
                        ), barrierDismissible: false) :Get.dialog(ConfirmationDialogWidget(
                          icon: Images.warning, title: 'are_you_sure_to_accept'.tr,
                          iDs: orderModel.groupOrder,
                          description:orderModel.collectionOrder==true?"thisOrderHaveManyOrderForSameUserUMustAcceptThem".tr: parcel ? 'you_want_to_accept_this_delivery'.tr : 'you_want_to_accept_this_order'.tr,
                          onYesPressed: () {
                            if(orderModel.collectionOrder==true){
                              for(int id in orderModel.groupOrder??[]){
                                orderController.acceptOrder(id, index, orderModel).then((isSuccess) {
                                  if(isSuccess) {
/*
                                  onTap();
*/
                              ...
                              orderController.acceptOrder(orderModel.mainOrder, index, orderModel).then((isSuccess) {
                                if(isSuccess) {
/*
                                onTap();
*/
