...
class ConfirmationDialogWidget extends StatelessWidget {
  ...
  const ConfirmationDialogWidget({super.key, required this.icon, this.iconSize = 50, this.title, required this.description, required this.onYesPressed,
    this.isLogOut = false, this.hasCancel = true, this.iDs});
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          if(iDs?.isNotEmpty==true)
            Text("IdOfOtherOrders".tr),
          if(iDs?.isNotEmpty==true)
          Padding(
            padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
            child: Text(iDs!.join(","), style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeLarge), textAlign: TextAlign.center),
          ),
